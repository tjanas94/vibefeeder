...
// customErrorHandler is a global error handler for Echo
func (a *App) customErrorHandler(err error, c echo.Context) {
	...
	if isHTMX {
		// For HTMX requests, use a simple error fragment
	...
	if err := c.Render(code, "", component); err != nil {
		a.Logger.Error("Failed to render error response", "error", err)
	}
}
